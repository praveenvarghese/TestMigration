...
scenario("Validate warnings are dispalyed at both page level and application level.", () => {
	...
	findDialog().clickNavigateToPageButton(1);
		...
		.should.beEqualTo({
			page: {
				...
				issues: [
					{
						title: "Table Sorting Configuration Needs Correction",
						description: `The table "SalesData" has a saved sorting configuration that may result in incorrect row or column sorting—especially when some rows or columns are hidden using the "Visibility" feature. Click 'Resolve' to fix this.`,
						buttonText: "Resolve",
					},
				],
			},
