...
scenario(
	"For Map loaded with Nodes, with Node-Sizes and tooltips annotations applied. Hover the nodes and assert the tooltip text seen.",
	() => {
		...
		findWidget("Airports")
			.getPoint("Groningen")
			.hover();
		findWidget("Airports").getTooltip().should.not.exist;

		// Amersfoort node has custom tooltip. Hover and assert tooltip is seen with relevant text.
		findWidget("Airports")
			.getPoint("Amersfoort")
			.hover();
		let tooltip = findWidget("Airports").getTooltip();
		...
		findWidget("Airports")
			.getPoint("Breda")
			.hover();
		tooltip = findWidget("Airports").getTooltip();
		tooltip.should.exist;
