...
scenario(
	"Test to verify that error message is displayed. if pageid type is sidepanel pagetype is configured in workflow.",
	() => {
		...
		// Entering a Grid type sidepanel PageId for a Step
		findWidget("Workflow Configuration")
			.getCell(5, 2)
		...
		findWidget("Workflow Configuration")
			.getCell(0, 2)
			.setValue("classicsidepanel");

		// Verify that a proper error message is logged
		getLogMessage()
			.getLastReportedLogMessage()
			.should.contain(
				'Workflow validation:In the "e2e Workflow Title" workflow, the "Step A" step has an invalid PageId specified.'
			);
